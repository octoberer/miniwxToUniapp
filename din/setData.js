...
export function convertSetData(input) {
    // 解析输入代码为 AST
    ...
    // 遍历 AST，找到 this.setData 调用并转换
    traverse(ast, {
        CallExpression(path) {
            const { callee, arguments: args } = path.node;

            // 检查是否是 this.setData 调用
            if (
                ...
                if (t.isObjectExpression(firstArg)) {
                    ...
                    firstArg.properties.forEach((property) => {
                        const key = property.key.name; // 属性名
                        const value = processValue(property.value); // 处理值

                        // 如果值是对象，递归处理
                        if (t.isObjectExpression(property.value)) {
                            // 遍历对象的键
                            property.value.properties.forEach((subProperty) => {
                                const subKey = subProperty.key.name; // 子属性名
                                const subValue = processValue(subProperty.value); // 处理子值
                                statements.push(
                                    t.expressionStatement(
                                        t.assignmentExpression(
                                            '=',
                                            t.memberExpression(
                                                t.identifier(`${key}.${subKey}`),
                                                t.identifier('value')
                                            ),
                                            subValue
                                        )
...
function processValue(value) {
    ...
    // 处理 MemberExpression（如 this.data.XXX）
    if (t.isMemberExpression(value)) {
        // 检查是否是 this.data.XXX 的形式
        if (
            t.isThisExpression(value.object) &&
            t.isIdentifier(value.property) &&
            value.property.name.startsWith('data.')
        ) {
            // 提取变量名（如 hasOpened），并转换为 hasOpened.value
            const propName = value.property.name.split('.').pop();
            return t.memberExpression(t.identifier(propName), t.identifier('value'));
        }
    }
...
if (t.isObjectMethod(prop)) {
    ...
    const modifiedBody = replaceThisAccess(methodBody, stateProperties);
    lifecycle.push({
        name: methodName,
        params: genParamsWithType(prop),
